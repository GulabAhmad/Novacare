...
const API_CONFIG = {
  openaiApiKey: process.env.NEXT_PUBLIC_OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY || '',
  openaiOrgId: process.env.NEXT_PUBLIC_OPENAI_ORG_ID || process.env.VITE_OPENAI_ORG_ID || '',
  vapiApiKey: process.env.NEXT_PUBLIC_VAPI_API_KEY || process.env.VITE_VAPI_API_KEY || '',
  vapiVoiceId: process.env.NEXT_PUBLIC_VAPI_VOICE_ID || process.env.VITE_VAPI_VOICE_ID || ''
};
    ...
    try {
      ...
      this.vapi.on("message", (message: any) => {
        ...
        if (message.type === 'transcript') {
          ...
          const formattedTranscript = message.role === 'assistant' 
      ...
      const usePremadeAssistant = process.env.NEXT_PUBLIC_USE_PREMADE_ASSISTANT === 'true' || process.env.VITE_USE_PREMADE_ASSISTANT === 'true';
      const assistantId = process.env.NEXT_PUBLIC_VAPI_ASSISTANT_ID || process.env.VITE_VAPI_ASSISTANT_ID;
      ...
      if (usePremadeAssistant && assistantId) {
        ...
        console.log('System Prompt:', this.config.systemPrompt || "You are a helpful sales training AI assistant. Keep responses concise and focused on the current topic.");
        try {
          await this.vapi.start({
            ...
            model: {
              ...
              organization: process.env.NEXT_PUBLIC_OPENAI_ORG_ID || process.env.VITE_OPENAI_ORG_ID,
              messages: [
                {
                  role: "system",
                  content: this.config.systemPrompt || "You are a helpful sales training AI assistant. Keep responses concise and focused on the current topic."
                }
              ],
              temperature: 0.7
            },
            ...
            clientMessages: ["transcript", "status-update", "model-output", "speech-update"],
